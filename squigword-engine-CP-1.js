...
// ============================================
// AUTO-GENERATED CONNECTED STROKES
// Entry snout at letter's natural entry height (not forced Y=183).
// No exit tail — bridging in flushWord() handles height transitions.
// Snout X capped: no further left than 15% of glyph width before origin.
// Hand-sculpted connectedStrokes (b, d, z) are preserved.
// ============================================
for (const letter of 'abcdefghijklmnopqrstuvwxyz') {
    const glyph = FONT_ALLURE[letter];
    if (!glyph) continue;
    if (letter === 'z') continue; // hand-sculpted
    delete glyph.connectedStrokes; // clear any stale programmatic data
    const connected = glyph.strokes.map((stroke, si) => {
        if (si > 0) return stroke; // secondary strokes (dots, crossbars) unchanged
        const entry = stroke[0];
        // Cap snout X: no further left than 15% of glyph width before origin
        ...
        // Snout at letter's natural entry height — bridge handles transitions
        const snoutY = entry[1];
        // Only add snout if meaningfully left of entry
        ...
        return [...stroke];
        // No exit tail — flushWord() adaptive bridging handles height transitions
    });
    glyph.connectedStrokes = connected;
    // Also generate connected finishing strokes where finishing variants exist
    ...
    function flushWord() {
        if (!useWordPath) {
            ...
            for (let i = 1; i < currentWordPrimary.length; i++) {
                ...
                // Band-aware bridging: connection shape adapts to height difference
                // between previous letter's exit and next letter's entry.
                if (dist < 5) {
                    ...
                    if (bDist <= 1) {
                        // Same band or one apart: flex endpoints toward each other
                        // One-apart gets wider flex for a gentle S-curve
                        ...
                        // Two bands apart (HIGH↔LOW): wider bridge with intermediate
                        // points that meet in the middle for a smooth S-curve
                        wordPath[wordPath.length - 1] = [
                            prev[0] + dx * 0.08,
                            prev[1] + dy * 0.06
                        ];
    ...
    for (const char of text) {
        ...
        for (let si = 0; si < activeStrokes.length; si++) {
            ...
            if (isSecondary && defer) {
                ...
                // Collect primaries for word-path concatenation (tagged with letter index + band info)
                currentWordPrimary.push({
                    path: transformedPath, li,
