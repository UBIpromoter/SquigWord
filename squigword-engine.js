...
const EMS_ALLURE = {
' ': {w:378},
'a': {w:526, d:'M 495 183 L 457 132 L 406 78.8 L 331 31.5 L 271 18.9 L 243 63 L 255 142 L 280 260 L 240 210 L 176 145 L 69.3 56.7 L -18.9 18.9 L -63 66.1 L -56.7 154 L 34.6 261 L 148 331 L 230 356 L 312 356 L 324 343'},
...
const STUB_BOUNDS = {
    ...
    'w': { stubStart: 0, stubEnd: 17 },
    'x': { stubStart: 0, stubEnd: 9 },
    'y': { stubStart: 0, stubEnd: 23 },
    'z': { stubStart: 0, stubEnd: 20 },
};
...
// Generate a smooth connector between two letters (in screen coordinates,
// after scaling/offset applied). Returns array of [x,y] points.
function generateConnectorPoints(exitX, exitY, exitAngle, entryX, entryY, entryAngle, nextCategory, connDrama) {
    if (connDrama == null) connDrama = 0.7; // default mid-range drama
    const dx = entryX - exitX, dy = entryY - exitY;
    const dist = Math.sqrt(dx*dx + dy*dy);
    if (dist < 5) return [];

    // Direct angle from exit to entry — the "ideal" straight-line direction
    const directAngle = Math.atan2(dy, dx);

    // Blend exit angle toward direct angle to prevent loops/kinks when the
    // stored meta angle doesn't match the actual path-end tangent.
    // 0.5 = equal blend of stroke tangent + direct direction
    const blendedExit = _blendAngle(exitAngle, directAngle, 0.5);

    // For category B letters (a, s) that enter from a high/looping position,
    // use a softened entry: blend actual entry angle toward the direct approach.
    // Old code forced a hard upward angle that created loops behind the letter.
    let effectiveEntry;
    if (nextCategory === 'B') {
        // Reverse of direct = the direction approaching from the exit side
        const approachAngle = directAngle + Math.PI;
        effectiveEntry = _blendAngle(entryAngle, approachAngle, 0.4);
    } else {
        effectiveEntry = entryAngle;
    }

    // Handle length scales with drama: 0=tight bridges (0.15), 1=flowing arcs (0.50)
    const hLen = dist * (0.15 + 0.35 * connDrama);
    const p0 = [exitX, exitY];
    const p1 = [exitX + Math.cos(blendedExit) * hLen, exitY + Math.sin(blendedExit) * hLen];
    const p2 = [entryX - Math.cos(effectiveEntry) * hLen, entryY - Math.sin(effectiveEntry) * hLen];
...
// Blend two angles (handles wrapping) — ratio 0 = angleA, 1 = angleB
function _blendAngle(a, b, ratio) {
    let diff = b - a;
    // Normalize to [-PI, PI]
...
for (const letter of 'abcdefghijklmnopqrstuvwxyz') {
    ...
    const connected = glyph.strokes.map((stroke, si) => {
        ...
        // Approach angle: blend horizontal with first segment direction to avoid Catmull-Rom overshoot
        const next = stroke.length > 1 ? stroke[1] : entry;
        const dx = next[0] - entry[0], dy = next[1] - entry[1];
        const segLen = Math.sqrt(dx * dx + dy * dy) || 1;
        // Snout points approach from left, with slight Y offset matching 30% of first segment's Y direction
        const yBlend = 0.3;
        const farDist = entry[0] - snoutFarX;
        const nearDist = entry[0] - snoutNearX;
        const snoutFarY = entry[1] - (dy / segLen) * farDist * yBlend;
        const snoutNearY = entry[1] - (dy / segLen) * nearDist * yBlend;
        if (snoutFarX < entry[0] - 8) {
            return [[snoutFarX, snoutFarY], [snoutNearX, snoutNearY], ...stroke];
        }
        return [...stroke];
    });
    ...
    if (glyph.finishingStrokes) {
        glyph.connectedFinishingStrokes = glyph.finishingStrokes.map((stroke, si) => {
            ...
            const snoutNearX = Math.max(entry[0] - 18, minSnoutX + 8);
            const next = stroke.length > 1 ? stroke[1] : entry;
            const dx2 = next[0] - entry[0], dy2 = next[1] - entry[1];
            const segLen2 = Math.sqrt(dx2 * dx2 + dy2 * dy2) || 1;
            const farDist2 = entry[0] - snoutFarX;
            const nearDist2 = entry[0] - snoutNearX;
            const snoutFarY = entry[1] - (dy2 / segLen2) * farDist2 * 0.3;
            const snoutNearY = entry[1] - (dy2 / segLen2) * nearDist2 * 0.3;
            if (snoutFarX < entry[0] - 8) {
                return [[snoutFarX, snoutFarY], [snoutNearX, snoutNearY], ...stroke];
            }
            return [...stroke];
...
// Letter zone classification for Layer 2 expressive parameters
const LETTER_ZONES = {
    descenders: new Set('gjpqy'),
    ascenders: new Set('bdfhklt'),
    xHeightTop: 365,       // top of body letters
    baseline: 183,         // ~center/connection height
    descenderFloor: -415,  // lowest descender (y, q)
    ascenderCeiling: 790,  // highest ascender (d)
};
...
function deriveHandPersonality(decPairs) {
    return {
        ...
        // Layer 2: Descender drama — how far g/j/p/q/y extend below baseline (1.0=normal, 1.3=wild)
        descenderDrama: map((decPairs[11] ^ decPairs[22]) & 0xFF, 0, 255, 0.95, 1.15),
        // Layer 2: Ascender reach — how far b/d/f/h/k/l/t extend above x-height (1.0=normal, 1.2=reaching)
        ascenderReach: map((decPairs[13] ^ decPairs[23]) & 0xFF, 0, 255, 0.95, 1.10),
        // Layer 2: Connector drama — swoopiness of inter-letter connectors (0=tight, 1=flowing)
        connectorDrama: map((decPairs[4] ^ decPairs[16]) & 0xFF, 0, 255, 0.4, 1.0),
        // Layer 2: Dot scale — size of dots on i, j, etc (0.8=small, 1.3=big)
        dotScale: map((decPairs[6] ^ decPairs[18]) & 0xFF, 0, 255, 0.85, 1.2),
        // Layer 2: Dot angle — slight rotation of dot strokes (radians, ±0.12)
        dotAngle: map((decPairs[7] ^ decPairs[19]) & 0xFF, 0, 255, -0.12, 0.12),
        // Layer 2: Dash jitter — position jitter for crossbars/dashes (px, 0=precise, 25=loose)
        dashJitter: map((decPairs[8] ^ decPairs[20]) & 0xFF, 0, 255, 0, 25),
    };
}
...
// Layer 2: Dot/dash personality — mutates transformed secondary stroke in-place.
// Dots (≤5 original points, compact extent): scaled + rotated around centroid.
// Dashes/crossbars (wider): jittered in position.
function _applySecondaryPersonality(path, hand, origLen) {
    if (path.length < 2) return;
    // Compute centroid
    let cx = 0, cy = 0;
    for (const [px, py] of path) { cx += px; cy += py; }
    cx /= path.length; cy /= path.length;
    // Compute extent to classify dot vs dash
    let minX = Infinity, maxX = -Infinity;
    for (const [px] of path) { minX = Math.min(minX, px); maxX = Math.max(maxX, px); }
    const extent = maxX - minX;
    const isDot = origLen <= 5 && extent < 40; // small compact stroke = dot
    if (isDot) {
        // Scale around centroid + rotate
        const sc = hand.dotScale || 1.0;
        const ang = hand.dotAngle || 0;
        const cosA = Math.cos(ang), sinA = Math.sin(ang);
        for (let i = 0; i < path.length; i++) {
            let dx = (path[i][0] - cx) * sc;
            let dy = (path[i][1] - cy) * sc;
            path[i][0] = cx + dx * cosA - dy * sinA;
            path[i][1] = cy + dx * sinA + dy * cosA;
        }
    } else {
        // Dash/crossbar: subtle position jitter
        const jit = hand.dashJitter || 0;
        if (jit > 0) {
            // Deterministic jitter from centroid position
            const seed = Math.floor(cx * 7 + cy * 13) & 0xFFFF;
            const jx = ((seed * 2654435761 >>> 16) / 65536 - 0.5) * jit;
            const jy = ((seed * 2246822519 >>> 16) / 65536 - 0.5) * jit * 0.4; // less vertical jitter
            for (let i = 0; i < path.length; i++) {
                path[i][0] += jx;
                path[i][1] += jy;
            }
...
function transformStroke(stroke, params) {
    const { glyphCenterX, glyphCenterY, loopWidth, loopHeight,
            ...
            scaleVar, cosR, sinR, yWobble, variance,
            zoneStretch } = params;
    ...
    return stroke.map(([fx, fy], ptIdx) => {
        ...
        // Layer 2: zone stretching — descenders/ascenders stretch beyond body zone
        // Applied in font space so it scales with the letter naturally.
        // Points inside x-height body (baseline to xHeightTop) are untouched.
        if (zoneStretch) {
            const { mult, anchorY } = zoneStretch;
            if (mult !== 1.0 && anchorY != null) {
                const dist = sy - anchorY;
                // Only stretch points on the "far" side of the anchor
                // For descenders: anchor = baseline, stretch points below (dist < 0)
                // For ascenders: anchor = xHeightTop, stretch points above (dist > 0)
                if ((zoneStretch.direction === 'below' && dist < 0) ||
                    (zoneStretch.direction === 'above' && dist > 0)) {
                    sy = anchorY + dist * mult;
                }
            }
        }
...
function layoutLine(text, canvasWidth, canvasHeight, type, config, forceScale) {
    ...
    // Adaptive spacing: connector-aware gaps between letters.
    // Letters with large vertical gaps (HIGH→LOW) get more horizontal room.
    const CONN_BASE_GAP = 40;    // minimum gap between letters (font units)
    const CONN_VERT_RATIO = 0.12; // extra gap per unit of vertical distance

    function _connectorGap(prevGlyph, curGlyph) {
        if (!prevGlyph || !curGlyph) return 0;
        const pm = prevGlyph.connectorMeta, cm = curGlyph.connectorMeta;
        if (!pm || !cm) return CONN_BASE_GAP;
        const vertDist = Math.abs(pm.exitY - cm.entryY);
        return CONN_BASE_GAP + vertDist * CONN_VERT_RATIO;
    }

    let totalFontWidth = 0;
    let prevGlyphForWidth = null;
    for (const char of text) {
        // Underscore = phantom space: use space width for layout
        const effectiveChar = char === '_' ? ' ' : char;
        const glyph = FONT[effectiveChar];
        if (!glyph) continue;
        if (char === ' ' || char === '_') { prevGlyphForWidth = null; totalFontWidth += glyph.width; continue; }
        const isUpper = char >= 'A' && char <= 'Z';
        const boldSpacing = isBoldType ? (isUpper ? 0.20 : 0.35) : 0;
        const gap = prevGlyphForWidth ? _connectorGap(prevGlyphForWidth, glyph) : 0;
        totalFontWidth += glyph.width * (1 + boldSpacing) + gap;
        prevGlyphForWidth = glyph;
    }
        ...
        if (!useWordPath) {
            ...
            for (let i = 1; i < currentWordPrimary.length; i++) {
                ...
                if (dist < 5) {
                    ...
                    // Compute actual exit tangent from transformed path endpoints
                    // (not stub-based meta angle which may differ from ramp endpoint)
                    const wpN = wordPath.length;
                    let actualExitAngle = prevCM.exitAngle;
                    if (wpN >= 3) {
                        const adx = (wordPath[wpN-1][0] - wordPath[wpN-3][0]) / 2;
                        const ady = (wordPath[wpN-1][1] - wordPath[wpN-3][1]) / 2;
                        actualExitAngle = Math.atan2(ady, adx);
                    } else if (wpN >= 2) {
                        actualExitAngle = Math.atan2(wordPath[wpN-1][1] - wordPath[wpN-2][1],
                                                     wordPath[wpN-1][0] - wordPath[wpN-2][0]);
                    }
                    // Compute actual entry tangent from next letter's transformed path start
                    let actualEntryAngle = nextCM.entryAngle;
                    if (nextPath.length >= 3) {
                        const adx = (nextPath[2][0] - nextPath[0][0]) / 2;
                        const ady = (nextPath[2][1] - nextPath[0][1]) / 2;
                        actualEntryAngle = Math.atan2(ady, adx);
                    } else if (nextPath.length >= 2) {
                        actualEntryAngle = Math.atan2(nextPath[1][1] - nextPath[0][1],
                                                      nextPath[1][0] - nextPath[0][0]);
                    }
                    // New connector joint system: smooth Bezier between exit and entry
                    const connPts = generateConnectorPoints(
                        prev[0], prev[1], actualExitAngle,
                        next[0], next[1], actualEntryAngle,
                        nextCM.category,
                        config.hand ? config.hand.connectorDrama : 0.7
                    );
                    if (connPts.length > 0) {
    ...
    let prevGlyphForGap = null; // tracks previous letter for adaptive spacing
    ...
    for (const char of text) {
        ...
        if (isCapital) {
            ...
            prevGlyphForGap = null; // capitals break the gap chain
        }

        // Adaptive connector gap: add space before this letter based on exit→entry distance
        if (prevGlyphForGap && !isCapital) {
            const preGap = _connectorGap(prevGlyphForGap, glyph);
            xOffset += preGap * scale;
        }
        ...
        for (let si = 0; si < activeStrokes.length; si++) {
            ...
            // Layer 2: compute zone stretch for descenders/ascenders
            let zoneStretch = null;
            if (hand) {
                const lc = char.toLowerCase();
                if (LETTER_ZONES.descenders.has(lc) && hand.descenderDrama !== 1.0) {
                    zoneStretch = { mult: hand.descenderDrama, anchorY: LETTER_ZONES.baseline, direction: 'below' };
                } else if (LETTER_ZONES.ascenders.has(lc) && hand.ascenderReach !== 1.0) {
                    zoneStretch = { mult: hand.ascenderReach, anchorY: LETTER_ZONES.xHeightTop, direction: 'above' };
                }
            }
            ...
            if (isSecondary && defer) {
                // Layer 2: Apply dot/dash personality to secondary strokes
                if (hand) _applySecondaryPersonality(transformedPath, hand, stroke.length);
...
window.SquigWord = {
    ...
    // Editor helper: connector gap in font units (matches _connectorGap in layoutLine)
    connectorGapFontUnits: function(prevGlyph, curGlyph) {
        if (!prevGlyph?.connectorMeta || !curGlyph?.connectorMeta) return 40;
        const vertDist = Math.abs(prevGlyph.connectorMeta.exitY - curGlyph.connectorMeta.entryY);
        return 40 + vertDist * 0.12;
    }
};
